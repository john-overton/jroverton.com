...
import { ChevronDown, ChevronRight, CircleHelp, Download, GripVertical, Play, Redo2, Save, Undo2 } from 'lucide-react';
...
export default function ShiftBidsPanel({ runs, depots, readonlyView, savedBidResult, onBidResultChange }: ShiftBidsPanelProps) {
  ...
  const { pushState, undo: undoBid, redo: redoBid, clearHistory, canUndo, canRedo } = useUndoRedo<BidResult>();
  const { toasts, showToast, dismissToast } = useToast();

  // Load saved bid result on mount
  useEffect(() => {
    if (initializedRef.current) return;
    if (savedBidResult) {
      initializedRef.current = true;
      setResult(savedBidResult);
      setConfig(savedBidResult.config);
  ...
  function handleBidUndo() {
    if (!result) return;
    const previous = undoBid(result);
    if (previous) {
      setResult(previous);
  ...
  function handleBidRedo() {
    if (!result) return;
    const next = redoBid(result);
    if (next) {
      setResult(next);
      persistResult(next);
    }
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    const violations = validateBlockMove(result, dragData.sourcePackageId, dragData.blocks, targetPackageId);
  ...
  function executeBlockMove(
    ...
    let updatedPackages = result.packages.map((pkg) => {
    ...
    let unassignedBlocks = [...result.unassigned_blocks];
    ...
    updatedPackages = updatedPackages.map((pkg) => ({
      ...pkg,
      type: (pkg.weekly_pay_hours >= result.config.fte_min_hours ? 'FTE' : 'PT') as BidType,
    }));

    const reranked = rankPackages(updatedPackages, result.config);
        ...
        <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
          <div className="flex items-center gap-2">
            <Button size="sm" onClick={handleGenerateClick} disabled={runs.length === 0 || readonlyView} type="button">
              ...
              {result ? 'Regenerate Bids' : 'Generate Bids'}
            </Button>
            ...
            <Button
              ...
              size="sm"
              onClick={handleSave}
              disabled={!result || readonlyView || saveStatus === 'saving'}
              type="button"
            >
            ...
            {result && !readonlyView && (
              <>
                <Button variant="ghost" size="icon-sm" onClick={handleBidUndo} disabled={!canUndo} title="Undo" type="button">
                  <Undo2 size={16} />
                </Button>
                <Button variant="ghost" size="icon-sm" onClick={handleBidRedo} disabled={!canRedo} title="Redo" type="button">
                  <Redo2 size={16} />
                </Button>
              </>
            )}
                  ...
                  <DroppablePackageBody packageId="unassigned">
                    <TableRow>
                      ...
                      <TableCell colSpan={9} className="text-xs font-medium text-cc-danger">
                        Unassigned Blocks ({result.unassigned_blocks.length})
                      </TableCell>
                    </TableRow>
                    {collapseRoutes(result.unassigned_blocks).map((route, idx) => {
                      const routeBlocks = getBlocksForCollapsedRoute({ daily_blocks: result.unassigned_blocks }, route);
                      return (
                        <DraggableRouteRow
