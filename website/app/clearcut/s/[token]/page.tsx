...
export default function ClearCutEditSessionPage({ params }: { params: { token: string } }) {
    ...
    const load = async () => {
      try {
        ...
        if (!res.ok || !payload?.ok) {
          throw new Error(payload?.error?.message ?? 'Failed to load edit session.');
        }
        ...
        if (!cancelled) {
          setState({
            status: 'error',
            message: err instanceof Error ? err.message : 'Failed to load edit session.',
          });
        }
      }
    };
  ...
  return (
    <main style={{ maxWidth: 900, margin: '0 auto', padding: '5rem 1.25rem 2rem' }}>
      <h1 style={{ fontSize: '2rem', marginBottom: '0.5rem' }}>ClearCut Edit Session</h1>
      <p style={{ color: '#666', marginBottom: '1rem' }}>
        URL mode: <code>/clearcut/s/{params.token}</code>
      </p>
